 ...
 *
 * <p>
 * 如果希望 JUnit Jupiter 在同一个测试实例上执行所有测试方法, 需使用 {@code @TestInstance(Lifecycle.PER_CLASS)} 对测试类进行注解.
 * 当使用这种模式时, 只创建测试类的一个实例并在测试之间重用它. 因此, 如果测试方法依赖于存储在实例变量中的状态, 则可能需要在
 * {@code @BeforeEach} 或 {@code @AfterEach} 方法中重置该状态
 * </p>
 *
 * <p>
 * 如果测试类是一个内部类, 正常情况下是不会执行其中注解为 {@link Test @Test} 的测试方法的, 需要在内部类上注解
 * {@link Nested @Nested}
 * </p>
 */
class LifecycleTest {
     ...
     *
     * <p>
     * 此时每个测试方法都会独立创建一个测试对象来执行, 测试之间完全隔离, 一个测试方法对当前测试类对象的影响不会传递给另一个测试方法
     * </p>
     */
    @Nested
