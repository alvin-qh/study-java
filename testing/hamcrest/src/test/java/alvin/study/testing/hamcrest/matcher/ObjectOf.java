 ...
 * </p>
 * <p>
 * 当前类型的超类 {@link TypeSafeDiagnosingMatcher} 表示一个 "类型安全"
 * 并且 "包含下一级 {@link Matcher}" 的类型, 相当于结合了
 ...
 *
 * @param <T> 原始对象类型, 通过 {@code mapper} 参数转化为
 *            {@code <R>} 类型对象
 * @param <R> 要匹配的对象类型
 */
public class ObjectOf<T, R> extends TypeSafeDiagnosingMatcher<T> {
    ...
    /**
     * 构造器, 传入 {@link Function} 用于对象转换, 传入 {@link Matcher}
     * 对象对转换后的对象进行匹配
     *
    ...
    /**
     * 构造一个 {@link Matcher} 对象, 将 {@code mapper} 参数的返回值通过
     * {@code matcher} 参数进行匹配后返回匹配结果
     *
     * @param <T>     原始对象类型, 通过 {@code mapper} 参数转化为
     *                {@code <R>} 类型对象
     * @param <R>     要匹配的对象类型
     ...
     */
    public static <T, R> Matcher<T> objectOf(
            Function<? super T, ? extends R> mapper,
            Matcher<? extends R> matcher) {
     ...
     *
     * <p>
     * 匹配失败时, 需要输出当前 {@link Matcher} 的期望信息, 即
     * {@code matcher} 参数的 {@code describeTo} 方法返回值
     * </p>
     */
    @Override
