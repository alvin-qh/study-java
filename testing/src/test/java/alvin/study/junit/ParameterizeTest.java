 ...
 *
 * <p>
 * 为了对一个方法进行全面的测试, 往往需要对其各种参数情况进行测试, 为了减少工作量, 可以用"参数化测试"对一组参数进行重复测试
 * </p>
 *
 * <p>
 * 参数化测试通过注解 {@link ParameterizedTest @ParameterizedTest} 来表示, 此时需要通过
 * {@link ValueSource @ValueSource} 等注解为测试指定一组参数, 这些参数将通过测试方法的对应参数传递;
 * {@link ValueSource @ValueSource} 注解具备一系列属性, 用于指定传递给测试的参数值
 *
 * <ul>
...
class ParameterizeTest {
     ...
     *
     * <p>
     * 通过 {@link ValueSource @ValueSource} 注解的 {@code ints} 属性指定一系列假设值, 这些值会通过
     * {@code input} 参数依次传递给测试方法, 以此验证测试方法是否针对所有假设的参数均可成立
     * </p>
     *
     * <p>
     ...
     *
     * <p>
     * 除过 {@link ValueSource @ValueSource} 注解的 {@code strings} 属性指定一系列假设字符串值外,
     * {@link NullAndEmptySource @NullAndEmptySource} 注解提供了 {@code null} 和
     * {@code ""} 空字符串值 (相当于
     * {@link org.junit.jupiter.params.provider.NullSource @NullSource} 和
     * {@link org.junit.jupiter.params.provider.EmptySource @EmptySource} 两个注解的组合)
     * </p>
     *
     ...
     *
     * <p>
     * 除过 {@link EnumSource @EnumSource} 注解的 {@code value} 属性指定一个枚举类型, 通过
     * {@code names} 属性指定要包含或排除的枚举名称, 通过 {@code mode} 属性指定对于 {@code names}
     ...
     *
     * <p>
     * 当 {@link EnumSource @EnumSource} 注解的 {@code names} 属性包含正则表达式通配符时, 可以指定
     * {@code mode} 属性为 {@link Mode#MATCH_ANY} 或 {@link Mode#MATCH_NONE},
     * 以产生匹配正则表达式的假设值
     * </p>
     *
     * <p>
     ...
     *
     * <p>
     * {@link CsvSource @CsvSource} 注解的 {@code value} 属性可以指定一组 csv 格式的数据集, 每一项相当于
     * csv 中的一行数据, 通过 {@code delimiter} 属性指定的字符进行分隔 (可选, 默认为 {@code ','} 分割)
     * </p>
     *
     * <p>
     * csv 格式数据按行依次传递给测试方法, 其中第一列数据传递给第一个参数, 第二列数据传递给第二个参数, 以此类推
     * </p>
     *
     ...
     *
     * <p>
     * {@link CsvFileSource @CsvFileSource} 注解的 {@code resources} 属性指定 csv 文件的路径
     * (相对于当前项目), 通过 {@code delimiter} 属性指定的字符进行分隔 (可选, 默认为 {@code ','} 分割), 通过
     * {@code numLinesToSkip} 属性的值指定要跳过的行数 (标题行)
     * </p>
     *
     * <p>
     * csv 文件数据按行依次传递给测试方法, 其中第一列数据传递给第一个参数, 第二列数据传递给第二个参数, 以此类推
     * </p>
     *
     * @param input    csv 中第一列数据
     * @param expected csv 中第二列数据
     */
    @CsvFileSource(resources = "/data/test.csv", delimiter = ',', numLinesToSkip = 1)
    @ParameterizedTest
     ...
     *
     * <p>
     * {@link MethodSource @MethodSource} 注解默认情况下会在当前类下找和被注解的测试方法同名的方法作为参数生成方法,
     * 也可以通过 {@code value} 属性指定特定的方法名称
     * </p>
     *
     * <p>
     * 参数生成方法必须为 {@code static} 方法, 且返回值必须是 {@code Stream<? extends Arguments>} 类型,
     * {@link Arguments} 对象标识一组参数
     * </p>
     *
     ...
     */
    @MethodSource("alvin.study.junit.parameterized.StringParams#blankStrings")
    @ParameterizedTest
     ...
     *
     * <p>
     * {@link ArgumentsSource} 和 {@link MethodSource} 类似, 只是前者是一个专用的参数生成接口类型, 且注解的
     * {@code value} 属性是指定参数生成类型的 {@link Class} 对象, 灵活度更高一些
     * </p>
     *
     ...
     *
     * <p>
     * 不同在于, 通过 {@link VariableSource @VariableSource} 注解将测试方法和参数提供方联系在一起
     * </p>
     *
     * @param input    第一个假设参数
     ...
     *
     * <p>
     * 本例中的斜杠分割日期格式通过 {@link SlashyDateConverter} 类型完成到 {@link LocalDate} 类型的转换, 通过
     * {@link ConvertWith @ConvertWith} 注解来指定转换类型
     * </p>
     *
     ...
     *
     * <p>
     * 对于一组 csv 假设数据, 通过 {@link ArgumentsAccessor} 参数类型可以完整的获取每一行测试参数, 这样对于列数比较多的
     * csv 数据, 就不必定义过多的测试参数来获取, 具体获取方法如下:
     *
     * <ul>
     * <li>
     ...
     * </li>
     * <li>通过 {@link ArgumentsAccessor#get(int, Class)} 可以获取第 n 列的指定类型数据</li>
     * </ul>
     * </p>
     *
     ...
     *
     * <p>
     * 对于一组 csv 假设数据, 通过 {@link AggregateWith @AggregateWith} 注解可以指定一个聚合器类型
     * {@link UserAggregator}, 通过该类型聚合 csv 数据形成指定类型的参数
     * </p>
     *
     * <p>
     * 由于 {@link User} 参数使用了 csv 的前两列数据, 所以无法直接获取第三列数据, 此时可通过
     * {@link ArgumentsAccessor} 类型参数来获取指定列的数据
     * </p>
     *
     ...
     *
     * <p>
     * 通过 {@link ParameterizedTest @ParameterizedTest} 注解的 {@code name}
     * 属性可以指定测试在测试报告中显式的名称, 其中的占位符可以为:
     *
     * <ul>
     * <li>
     * <code>{index}</code> 占位符, 将替换为调用索引, 简单地说, 第一次执行的调用索引为 {@code 1}, 第二次执行的调用索引为
     * {@code 2}, 依此类推
     * </li>
     * <li>
