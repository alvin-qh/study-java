 ...
 *
 * <p>
 * {@link DisableIfTestFails @DisableIfTestFails} 可以在某个前置测试失败后, 禁止执行后续的测试
 * <ul>
 * <li>
 * 一般而言, 如果一系列测试通过 {@link Order @Order} 注解指定了执行顺序, 则前置的测试会作为后续测试正确的条件,如果前置测试失败,
 * 则后续的测试也无必要执行, 此时可通过 {@link DisableIfTestFails @DisableIfTestFails} 达成此目标
 * </li>
 * <li>
 ...
 *
 * <p>
 * {@link DisableIfDisplayName @DisableIfDisplayName} 注解将根据测试的显示名称进行匹配, 如果满足条件, 则该测试被禁用
 * <ul>
 * <li>
 * 该注解通过 {@code contains} 和 {@code matches} 两个属性来匹配测试名称, 前者匹配名称中包含的内容, 后者通过一个正则表达式匹配名称
 * </li>
 * <li>
 * 通常情况下, 该注解配合参数化注解 {@link ParameterizedTest @ParameterizedTest} 一同使用, 即在动态生成测试显示名称时,
 * 过滤掉其中的一部分测试执行
 * </li>
 * </ul>
 * </p>
 *
 * <p>
 * {@link DisableIfArgument @DisableIfArgument}, {@link DisableIfAnyArgument}, {@link DisableIfAllArguments} 等注解用于
 * 参数化测试中, 可以在参数值为特定情况下, 禁用此次测试
 * <ul>
 * <li>
 * 某些时候, 特定的参数会导致异常, 这类参数应在专门的异常测试环节进行, 而正常测试环境可以过滤掉这类参数. 如果测试集数据量较大, 可以通过注解在
 * 测试执行环节进行过滤
 * </li>
 * <li>
 * 这一系列注解过滤主要通过注解的两个属性: {@code contains} 和 {@code matches} 来设定, 前者匹配参数包含的内容, 后者根据一个正则表达式对
 * 参数进行匹配
 * </li>
 * <li>
 * {@link DisableIfArgument @DisableIfArgument} 注解用于对指定参数进行匹配, 通过其 {@code name} 或 {@code index} 属性指定具体
 * 的参数
 * </li>
 * <li>
 * {@link DisableIfAnyArgument} 和 {@link DisableIfAllArguments} 两个注解用于对所有参数进行匹配, 前者表示任意参数满足过滤条件, 后者
 * 表示所有参数满足过滤条件
 * </li>
 * </ul>
 * </p>
...
class DisabledTest {
     ...
     *
     * <p>
     * 本例中, 通过 {@link ValueSource @ValueSource} 注解来向 {@code x} 参数注入一系列值, 并通过 {@code x} 参数值生成测试显示名称,
     * 通过 {@link DisableIfDisplayName @DisableIfDisplayName} 注解对测试名称匹配正则表达式的情况进行过滤
     * </p>
     *
     ...
     *
     * <p>
     * 本例中, 通过 {@link CsvSource @CsvSource} 注解来向 {@code x} 参数注入一系列值, 并通过
     * {@link DisableIfArgument @DisableIfArgument} 注解对 {@code x} 参数值包含特定内容的情况进行过滤
     * </p>
     *
     * <p>
     * 本例中使用 {@link DisableIfArgument @DisableIfArgument} 注解的 {@code name} 属性来指定要匹配的参数, 即按名称匹配参数,
     * 对于 JDK 14 以上的 Java 版本, 需要在编译时打开 {@code -parameters}, 参考: {@code build.gradle} 文件以及 {@code pom.xml}
     * 文件
     * </p>
     *
     ...
     *
     * <p>
     * 本例中使用 {@link DisableIfArgument @DisableIfArgument} 注解的 {@code index} 属性来指定要匹配的参数, 即按索引匹配参数
     * </p>
     *
     * @param x 通过 {@link CsvSource} 注解注入的一系列参数
     */
    ...
    /**
     * {@link DisableIfArguments @DisableIfArguments} 注解用来组织一组 {@link DisableIfArgument @DisableIfArgument} 注解,
     * 对多个参数进行过滤
     *
     * @param x 通过 {@link CsvSource} 注解注入的一系列参数
     */
     ...
     *
     * <p>
     * 本例中测试方法有两个参数 {@code x} 和 {@code n}, 这两个参数中任意参数的值符合指定条件, 当前测试被禁用
     * </p>
     *
     * @param x 通过 {@link CsvSource} 注解注入的一系列参数
     ...
     *
     * <p>
     * 本例中测试方法有两个参数 {@code x} 和 {@code n}, 这两个参数的值都符合指定条件时, 当前测试被禁用
     * </p>
     *
     * @param x 通过 {@link CsvSource} 注解注入的一系列参数
     * @param n 通过 {@link CsvSource} 注解注入的一系列参数
