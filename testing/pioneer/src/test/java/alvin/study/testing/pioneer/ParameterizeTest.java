 ...
 *
 * <p>
 * {@link CartesianArgumentsSource @CartesianArgumentsSource}
 * 注解用于指定实际生成参数值的类, 本例中通过 {@link NumberSourceProvider}
 * 类来实际产生参数值
 * </p>
 *
 * <p>
 * 参考 {@link ParameterizeTest#cartesian_shouldGenerateArgumentsByAnnotation(int)}
 * 测试方法
 * </p>
 */
 ...
 * <p>
 * {@link CartesianArgumentsSource @CartesianArgumentsSource}
 * 注解用于指定实际生成参数值的类, 本例中通过 {@link NumberArgumentSourceProvider}
 * 类来实际产生参数值
 * </p>
 *
 * <p>
 * 为了简单起见, 本例只使用一种参数产生规则 {@link NumberArgumentSource#value()},
 * 实际使用时, 可以定义任意属性来产生所需的参数值
 * </p>
 *
 * <p>
 * 参考
 * {@link ParameterizeTest#cartesian_shouldGenerateAllArgumentsByAnnotation(int, int)}
 * 测试方法
 * </p>
 */
 ...
 *
 * <p>
 * 该类型需要实现 {@link CartesianParameterArgumentsProvider} 接口,
 * 其泛型参数指定了要生成参数的类型, 通过实现
 * {@link CartesianParameterArgumentsProvider#provideArguments(ExtensionContext, Parameter)}
 * 方法来产生参数集合
 * </p>
 *
 * <p>
 * 参考
 * {@link ParameterizeTest#cartesian_shouldGenerateArgumentsByAnnotation(int)}
 * 测试方法
 * </p>
 */
...
/**
 * 为 {@link NumberArgumentSource @NumberArgumentSource}
 * 注解的方法所有参数实际产生参数值的类
 *
 * <p>
 * 该类型需要实现 {@link CartesianMethodArgumentsProvider} 接口,
 * 通过实现 {@link CartesianMethodArgumentsProvider#provideArguments(ExtensionContext)}
 * 方法来为测试方法的所有参数产生参数值
 * </p>
 *
 * <p>
 * 为了简单起见, 本例为所有的参数都按同一种规则产生参数值
 * ({@link NumberArgumentSource#value()}), 实际使用时可以产生任何类型参数的组合
 * </p>
 *
 * <p>
 * 为了能获取到注解对象, 本例实现了 {@link AnnotationConsumer} 接口,
 * 其 {@link AnnotationConsumer#accept(Object)} 方法将测试方法上的
 * {@link NumberArgumentSource @NumberArgumentSource} 注解对象作为参数传递
 * </p>
 *
 * <p>
 * 参考
...
class NumberArgumentSourceProvider implements CartesianMethodArgumentsProvider,
    ...
    // NumberArgumentSource 注解的 value 属性值,
    // 作为产生参数值的规则
    private int[] numbers;
     ...
     * 传递注解对象
     *
     * @param source 定义在测试方法上的
     *               {@link NumberArgumentSource @NumberArgumentSource}
     *               注解对象
     */
    @Override
     ...
     *
     * @param context 测试方法的上下文对象, 包括测试方法的各类信息
     * @return 生成的参数值集合的 {@link ArgumentSets} 对象,
     *         包括测试方法的所有参数值
     */
    @Override
...
class ParameterizeTest {
    /**
     * 为 {@link #cartesian_shouldGenerateArgumentsByFactoryMethod(
     * int, String, TestEnum)} 测试方法提供测试参数
     *
     * <p>
     * 注意, 通过 {@link IntStream#range(int, int)} 得到的结果无法作为参数传递,
     * 需要通过 {@link IntStream#boxed()} 方法转为 {@code Stream<Integer>} 类型
     * </p>
     *
     * <p>
     ...
     * {@code value} 属性必须为
     * {@link org.junit.jupiter.api.TestInstance.Lifecycle#PER_CLASS Lifecycle.PER_CLASS}, 即:
     ...
     * <p>
     * 本方法为
     * {@link #json_shouldDeserializedObjectFromJsonInFile(User)}
     * 方法提供前置文件
     * </p>
     */
    @BeforeAll
     ...
     *
     * <p>
     * 通过 {@link Values @Values} 注解设定的两个参数 {@code x} 和 {@code y}
     * 会将以 {@code (x=1, y=3)}, {@code (x=1, y=4)}, {@code (x=2, y=3)}
     * 以及 {@code (x=2, y=4)} 的组合进行参数传递, 共执行 4 此测试
     * </p>
     *
     * <p>
     * {@link CartesianTest @CartesianTest} 注解的 {@code name}
     * 属性可以指定本测试每次执行的名称, 便于在日志中进行观察, 可以使用的
     * 占位符包括 <code>{index}</code>, 表示测试的索引编号;
     * <code>{0}, {1}, ..., {n}</code>, 表示传入的第几个参数
     * </p>
     *
     * @param x 第一组参数
     * @param y 第二组参数
     ...
     *
     * <p>
     * {@link IntRangeSource @IntRangeSource} 注解用于设置一系列在指定范围内的参数,
     * 类似功能的注解还包括
     ...
     * {@link org.junitpioneer.jupiter.params.DoubleRangeSource.DoubleRangeSources @DoubleRangeSources}
     * 注解组合多个范围设置, 进行批量设置
     * </p>
     *
     * <p>
     * {@link Enum @Enum} 注解的作用是遍历一个枚举的所有值, 其 {@code value}
     * 属性表示要遍历的枚举的类型, 当枚举类型和参数类型一致时可以省略 (例如本例可以省略),
     * 否则必须显式声明, 例如: {@code @Enum(ChronoUnit.class) TemporalUnit unit}
     * 参数
     * </p>
     *
     * <p>
     * {@link Enum @Enum} 注解具备 {@code names} 属性,
     * 默认情况表示要包含的枚举名称, 例如:
     * <code>
     * @Enum(names = { "DAYS", "HOURS" }) ChronoUnit unit
     * </code>
     * 参数, 表示只包含 {@code DAYS} 和 {@code HOURS} 两项
     * </p>
     *
     * <p>
     * {@link Enum @Enum} 注解具备 {@code mode} 属性, 取值为 {@link Enum.Mode} 枚举,
     * 表示 {@code names} 属性的含义, 默认为 {@link Enum.Mode#INCLUDE}, 表达如下:
     *
     * <ul>
     * <li>
     ...
     * </li>
     * <li>
     * {@link Enum.Mode#MATCH_ALL}: 表示 {@code names} 为一组正则表达式,
     * 取匹配所有正则表达式的枚举项
     * </li>
     * <li>
     * {@link Enum.Mode#MATCH_ANY}: 表示 {@code names} 为一组正则表达式,
     * 取匹配任一正则表达式的枚举项
     * </li>
     * </ul>
     * </p>
     ...
     * <p>
     * 测试参数的产生参考
     * {@link NumberSourceProvider#provideArguments(ExtensionContext, Parameter)}
     * 方法
     * </P>
     *
     * @param x 测试参数值
    ...
    /**
     * 通过 {@link NumberArgumentSource @NumberArgumentSource} 注解方法,
     * 为测试方法的每个参数产生一组测试值
     *
     * <p>
     * 测试参数的产生参考
     * {@link NumberArgumentSourceProvider#provideArguments(ExtensionContext)}
     * 方法
     * </P>
     *
     * <p>
     * 本例中为简单起见, 为每个参数生成值的规则是相同的, 所以两个参数都会产生
     * <code>{1, 2, 3}</code> 这三个值, 根据笛卡尔积的组合原则, 会产生
     * {@code (1, 1)}, {@code (1, 2)}, {@code (1, 3)}, {@code (2, 1)},
     * {@code (2, 2)}, {@code (2, 3)}, {@code (3, 1)}, {@code (3, 2)}
     * 和 {@code (3, 3)} 共 9 组参数值, 分别传递给 {@code x} 和 {@code y}
     * 参数
     * </p>
     *
     * @param x 测试参数值
     ...
     *
     * <p>
     * 使用 {@link ParameterizedTest @ParameterizedTest} 注解配合方法上的
     * {@link IntRangeSource} 注解, 可以产生一个在指定范围内执行的测试
     * </p>
     *
     * <p>
     * 对应的属性为:
     ...
     * <li>{@code to} 范围结束值</li>
     * <li>
     * {@code closed} 范围是否为闭区间, 默认为 {@code false}, 如果是闭区间,
     * 则参数取值为 {@code from <= value <= to}, 否则为
     * {@code from <= value < to}
     * </li>
     * <li>
     * {@code step} 步长值, 即参数在区间内每次增加的值, 默认为 {@code 1}
     * </li>
     * </ul>
     * </p>
     *
     ...
     * </li>
     * <li>
     * {@link org.junitpioneer.jupiter.params.FloatRangeSource FloatRangeSource}
     * </li>
     * <li>
     * {@link org.junitpioneer.jupiter.params.DoubleRangeSource DoubleRangeSource}
     * </li>
     * </ul>
     * </p>
     *
     * <p>
     * 规定为: {@link IntRangeSource @IntRangeSource} 等范围定义注解只能包含一次,
     * 且只能包含一个对应参数
     * </p>
     *
    ...
    void json_shouldDeserializedObjectFromJsonInResource(Group group) {
        // 确认 JSON 反序列化结果正确
        then(group).extracting("id", "name")
                .isNotEqualTo(tuple(1001, "Students"));
     ...
     *
     * <p>
     * 本例中通过 {@link #createJsonFile()} 方法临时创建了文件并写入
     * JSON 内容, 在所有测试结束后, 通过 {@link #deleteJsonFile()}
     * 方法删除该文件
     * </p>
     *
     * @param user 从 JSON 反序列化后得到的对象
     ...
     *
     * <p>
     * 如果无法 (或不必) 将 JSON 内容反序列化为对象,
     * 可以通过 {@link Property @Property} 注解指定获取 JSON 指定字段的参数
     * </p>
     *
     ...
     *
     * <p>
     * {@link JsonSource @JsonSource},
     * {@link JsonClasspathSource @JsonClasspathSource} 以及
     * {@link JsonFileSource @JsonFileSource} 三个注解的 {@code data}
     * 属性可以指定一个 JSON 字段名, 这样就只会将该字段的内容作为参数进行传递
     * </p>
     *
     * <p>
     * 注意, 如果只获取 JSON 的部分属性, 则无法将其反序列化为某个类型的对象,
     * 必须通过 {@link Property @Property} 对应指定属性名进行取值,
