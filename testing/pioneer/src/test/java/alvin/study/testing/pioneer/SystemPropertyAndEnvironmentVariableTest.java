 ...
 *
 * <p>
 * JVM 在启动时, 会读取指定的系统属性, 以 {@link java.util.Properties Properties}
 * 对象存储, 可通过
 *
 * <ul>
 ...
 *
 * <p>
 * 另外, JVM 也会读取当前设置的环境变量, 通过 {@link System#getenv()} 和
 * {@link System#getenv(String)} 方法进行读取 (JDK 不允许直接修改环境变量的值)
 * </p>
 *
 * <p>
 * 在测试过程中, 有时候需要临时 Mock 一些系统属性和环境变量的值, 但通过 Mockito
 * 之类的库比较难以实现, 此时可以通过本例中提供的方法进行
 * </p>
 *
 * <p>
 ...
 * </li>
 * <li>
 * {@link SetSystemProperties @SetSystemProperties},
 * 临时批量设置一系列系统属性
 * </li>
 * <li>
 * {@link ClearSystemProperties @SetSystemProperties},
 * 临时批量设置一系列系统属性
 * </li>
 * </ul
 * <p>
 *
 * <p>
 * Pioneer 通过反射修改 JVM 中存储的环境变量值. 自 JDK 17 开始,
 * 无法通过反射修改 JDK 内部状态, 所以和<b>环境变量修改</b>相关的测试会失败,
 * 抛出如下异常:
 *
 * <div>
 ...
 *
 * <div>
 * Java 认为环境变量不应当改变, 所以一般情况下不要使用
 * {@link ClearEnvironmentVariable @ClearEnvironmentVariable} 和
 * {@link SetEnvironmentVariable @SetEnvironmentVariable} 这一类注解,
 * 应当在 IDE 或者测试启动参数中设置正确的环境变量, 否则可能会引发未知的测试错误
 * </div>
 *
 * <div>
 * 为了让本例可以正确运行, 需要在 {@code jvmArgs} 参数中允许修改指定 Java
 * 模块的 JDK 内部状态, 分以下几个环节修改:
 * </div>
 *
 * <ul>
 * <li>
 ...
 * </li>
 * <li>
 * 对于 MAVEN 工具, 需修改 {@code pom.xml} 文件, 为
 * {@code maven-surefire-plugin} 插件增加 {@code <argLine>} 节点
 *
 * <pre>
 ...
 * </li>
 * <li>
 * 对于 GRADLE 工具, 需修改 {@code build.gradle} 文件, 为 {@code test}
 * 任务增加 {@code jvmArgs} 配置
 *
 * <pre>
...
class SystemPropertyAndEnvironmentVariableTest {
    ...
    /**
     * 测试通过 {@link ClearSystemProperty @ClearSystemProperty}
     * 注解删除系统属性值
     */
    @Test
    ...
    /**
     * 测试通过 {@link SetSystemProperty @SetSystemProperty}
     * 注解设置新的系统属性值
     */
    @Test
    ...
    /**
     * 测试通过 {@link ClearSystemProperties @ClearSystemProperties}
     * 注解批量删除指定的系统属性
     */
    @Test
