 ...
 * <ul>
 * <li>{@code value} 属性表示重试次数, 在测试失败达到此次数前成功一次则算测试通过, 否则测试失败</li>
 * <li>{@code maxAttempts} 含义同 {@code value} 属性, 表示测试总次数</li>
 * <li>{@code minSuccess} 表示测试至少要成功的次数, 该值必须小于 {@code value} 属性</li>
 * <li>{@code suspendForMs} 测试失败后, 和下次重试之间间隔的时间</li>
 * <li>{@code onExceptions} 一个异常类型, 表示只有抛出此类型异常才进行重试</li>
 * </ul>
 * </p>
 */
...
class RetryingFailingTest {
     ...
     *
     * <p>
     * 本例共执行 {@code 3} 次测试, 前两次抛出 {@link IllegalAccessException} 异常
     * </p>
     */
