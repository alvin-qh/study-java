 ...
 *
 * <p>
 * {@code groupingBy} 的主要作用是对一个 {@link java.util.stream.Stream Stream} 对象进行分组, 返回一个 {@link Map} 对象,
...
class GroupingByTest {
    ...
    /**
     * 测试 {@link Collectors#groupingBy(java.util.function.Function) Collectors.groupingBy(Function)}
     * 方法, 对集合数据进行简单分组
     *
     * <p>
     * 根据一个 {@link java.util.function.Function Function} 对象产生分组值, 进行分组, 返回一个 {@link Map} 结果, 其中
     ...
     *
     * <p>
     * 本例中通过一个 {@code Record} 类型产生一个联合 {@code Key} 作为分组依据, 参考 {@link BlogPost.TypeAndLikeKey} 类型
     * </p>
     *
     * <p>
     ...
     * Collectors.groupingBy(Function)} 方法将统一分组的 内容存放在一个 {@link List} 集合中, 若希望把分组存放在其它类型集合中,
     * 则可以通过 {@link java.util.stream.Collector Collector} 参数来指定
     * </p>
     *
     * <p>
     * 本例中将分组内容最终存放在 {@link Set} 集合中, 所以为 {@code groupingBy} 方法的第二个
     * {@link java.util.stream.Collector Collector} 参数传入 {@link Collectors#toSet()} 参数
     * </p>
     *
     * <p>
     ...
     *
     * <p>
     * 要在一次分组的基础上在此进行分组, 只需要在 {@code groupingBy} 方法的第二个 {@link java.util.stream.Collector Collector}
     * 参数传递另一个 {@code groupingBy} 方法即可
     * </p>
     *
     * <p>
     * 本例中的分组结果为 {@code Map<BlogPostType, Map<Integer, List<BlogPost>>>} 类型, 即先以 {@code BlogPostType} 进行分组,
     * 在每个分组内再以"点赞数"进行二次分组
     * </p>
     */
    @Test
     ...
     * <p>
     * 测试
     * {@link Collectors#averagingInt(java.util.function.ToIntFunction) Collectors.averagingInt(ToIntFunction)} 算子, 对
     * 分组结果求平均值
     * </p>
     *
     * <p>
     * 如果用于计算平均值的数据是浮点类型, 则可通过
     * {@link Collectors#averagingDouble(java.util.function.ToDoubleFunction) Collectors.averagingInt(ToDoubleFunction)}
     * 方法进行
     * </p>
     *
     * <p>
    ...
    /**
     * 测试 {@link Collectors#summingInt(java.util.function.ToIntFunction) Collectors.summingInt(ToIntFunction)}
     * 算子, 对分组结果进行求和计算
     *
     * <p>
     * 如果用于求和的数据是浮点类型, 则可以使用
     * {@link Collectors#summingDouble(java.util.function.ToDoubleFunction) Collectors.summingDouble(ToDoubleFunction)}
     * 方法进行
     * </p>
     *
     * <p>
    ...
    /**
     * 测试 {@link Collectors#maxBy(Comparator)}/{@link Collectors#minBy(Comparator)} 算子, 对分组结果求最大/最小值
     *
     * <p>
     ...
     *
     * <p>
     * 本例中的分组结果为 {@code Map<BlogPostType, Optional<BlogPost>>} 类型, 即点赞数最多或最少的 {@link BlogPost} 对象,
     ...
     *
     * <p>
     * 本例中的分组结果为 {@code Map<BlogPostType, IntSummaryStatistics>} 类型, 即根据帖子分类统计的点赞数各项指标
     * </p>
     */
    @Test
    ...
    /**
     * 测试 {@link Collectors#collectingAndThen(java.util.stream.Collector, java.util.function.Function)
     * Collectors.collectingAndThen(Collector, Function)} 方法, 在分组的基础上进行计算
     *
     * <p>
     * 本例中的分组结果为 {@code Map<BlogPostType, BlogPost.PostCountTitlesLikesStats>} 类型, 即根据帖子分类后产生的统计结果
     * </p>
     */
    @Test
     ...
     * {@link java.util.function.BinaryOperator BinaryOperator} 类型参数, 其作用是当集合中的两个元素得到同一个 {@code Key} 值时,
     * 如何合并这两个元素
     * </p>
     *
     * <p>
     * 本例的分组结果为 {@code Map<BlogPostType, BlogPost.TitlesBoundedSumOfLikes>}, 表示按帖子类型分组后, 各组的统计值
     * </p>
     */
    @Test
    ...
    /**
     * 测试 {@link Collectors#mapping(java.util.function.Function, java.util.stream.Collector)
     * Collectors.mapping(Function, Collector)} 方法, 对分组结果进行类型转换
     *
     * <p>
     * 本例的分组结果为 {@code Map<BlogPostType, String>}, 表示按帖子类型分组后, 将分组结果转为 {@link String} 类型
     * </p>
     */
    @Test
    ...
    /**
     * 测试 {@link Collectors#filtering(java.util.function.Predicate, java.util.stream.Collector)
    ...
    /**
     * 测试 {@link Collectors#flatMapping(java.util.function.Function, java.util.stream.Collector)
