 ...
 *
 * <p>
 * 一些时候, 某个类型的实现具备多个构造器, 在产生对象时无法确定使用哪个构造器. 此时,
 * 可以将类型和目标类型的指定构造器进行绑定, 通过被绑定的构造器产生所需的对象
 * </p>
 *
 * <p>
 * 所谓绑定, 即将一个抽象类型通过某种范围定义, 和其具体的实现类型或实现类型的一个具体实例进行绑定,
 * 如此以来当需要注入该抽象类型的实例时, Guice
 * 框架会从容器中找寻最符合注入场景范围定义的被绑定的类型或实例, 并将其产生的实例对象
 * (或其实例对象本身) 注入到目标对象中
 * </p>
 *
 * <p>
...
public class ConstructorBindingModule extends AbstractModule {
     ...
     *
     * <p>
     * {@link com.google.inject.binder.AnnotatedBindingBuilder#toConstructor(
     * java.lang.reflect.Constructor) AnnotatedBindingBuilder.toConstructor(Constructor)}
     * 方法表示将绑定的类型通过一个注解和目标进行绑定, 即当目标类型有多个构造器方法时,
     * 选择哪个对目标类型进行实例化, 返回一个
     ...
     *
     * <p>
     * {@code bind(BindDemo.class).toConstructor(constructor)} 这段代码说明的是,
     * 当注入的 {@link BindDemo} 类型字段时, 会通过给定的构造器方法对象实例化实际的
     * {@link BindDemoImpl} 类型对象
     * </p>
     */
    @Override
