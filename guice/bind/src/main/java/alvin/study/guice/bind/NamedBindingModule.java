 ...
 *
 * <p>
 * 和通过注解标记绑定关系类似, 可以通过命名的方式为绑定关系设置名称, 从而在注入的时候通过不同的名称诸如不同的实例.
...
public class NamedBindingModule extends AbstractModule {
     ...
     *
     * <p>
     * <code>bind(BindDemo.class).annotatedWith(Names.named("A")).to(BindDemoA.class)</code>
     * 这段代码说明的是将 {@link BindDemo} 接口通过名称 {@code "A"} 绑定到 {@link BindDemoA} 类型上,
     * 所以当注入同样标记为 {@code "A"} 的字段时, 会注入 {@link BindDemoA} 类型对象
     * </p>
     */
    @Override
