 ...
 * 如此以来当需要注入该抽象类型的实例时,
 * Guice 框架会从容器中找寻最符合注入场景范围定义的那个被绑定的类型或实例,
 * 并将其产生的实例对象 (或其实例对象本身) 注入到目标对象中
 * </p>
 *
 * <p>
...
public class ProviderBindingModule extends AbstractModule {
     ...
     * AnnotatedBindingBuilder.toProvider(Class)}
     * 方法表示将给定的类型绑定到一个实际的类型上, {@code toProvider} 方法的参数是一个
     * {@code Class<? extends Provider<? extends T>>} 类型的参数, 返回一个
     ...
     * <p>
     * <code>
     * bind(BindDemo.class).toProvider(BindDemoProvider.class)
     * </code>
     * 这段代码说明的是将 {@link BindDemo} 类型绑定到 {@link BindDemoProvider}
     * 类型上, 由 {@link BindDemoProvider#get()} 方法提供 {@link BindDemo}
     * 类型对象
     * </p>
     */
    @Override
