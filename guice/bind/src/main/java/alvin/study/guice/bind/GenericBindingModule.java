 ...
 *
 * <p>
 * 对于一个确定类型, 可以直接将其绑定到确定的目标类型上, 但对于一个泛型类型,
 * 不同的泛型参数意味着绑定的目标可能有所不同
 * </p>
 *
 * <p>
...
public class GenericBindingModule extends AbstractModule {
     ...
     *
     * <p>
     * {@link AbstractModule#bind(TypeLiteral)} 用于绑定一个泛型类型,
     * 返回一个 {@link com.google.inject.binder.AnnotatedBindingBuilder
     * AnnotatedBindingBuilder} 接口对象, 用于进一步配置类型绑定的目标
     * </p>
     *
     * <p>
     * {@link com.google.inject.binder.AnnotatedBindingBuilder#to(TypeLiteral)
     * AnnotatedBindingBuilder.to(TypeLiteral)}
     * 方法表示将给定的泛型类型绑定到一个实际的泛型类型上, 返回一个
     ...
     * AnnotatedBindingBuilder.toInstance(Object)}
     * 方法表示将给定的泛型类型绑定到一个实际的对象上
     * </p>
     *
     * <p>
     * <code>
     * bind(new TypeLiteral&lt;Set&lt;Integer&gt;&gt;() {}).to(new TypeLiteral&lt;HashSet&lt;Integer&gt;&gt;() {})
     * </code>
     ...
     * </code>
     * 这段代码说明的是, 当注入具备泛型参数的 {@code Set<Double>} 类型字段时,
     * 实际会注入一个具体的 {@code TreeSet<Double>} 类型对象
     * </p>
     */
    @Override
     ...
     *
     * <p>
     * 对于泛型绑定, 也可以通过 {@link Provides @Provides} 注解标注一个方法,
     * 该方法的返回值即为要绑定的泛型类型, 返回值即为绑定的目标对象
     * </p>
     */
    @Provides
    @Singleton
