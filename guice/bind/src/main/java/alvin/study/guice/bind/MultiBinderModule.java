 ...
 *
 * <p>
 * 多实例绑定, 即把同一类型 (或名称) 的实例绑定在一个集合对象中. 根据需要不同, 具备
 * {@link Multibinder} 和 {@link MapBinder} 两种方式可以选择, 前者的注入结果是一个
 * {@link java.util.Set Set} 集合, 后者是一个 {@link java.util.Map Map} 集合
 * </p>
 *
 * <p>
 ...
 * Guice.createInjector(Module...)} 方法创建的注入器对象可以根据模块设置的
 * 绑定关系, 对目标对象进行注入操作, 即通过
...
public class MultiBinderModule extends AbstractModule {
     ...
     *
     * <p>
     * {@link Multibinder} 对象中可以添加多个绑定关系, 并在容器中产生一个 {@link java.util.Set}
     * 集合, 注入该集合, 集合中的各个元素即为通过设置的绑定关系产生的对象实例
     * </p>
     *
     * <p>
     * {@link MapBinder} 的作用和 {@link MapBinder} 类似, 最终注入为一个
     * {@link java.util.Map Map} 集合, 区别是给了每个绑定关系一个 {@code Key} 值
     * </p>
     *
     * <p>
     ...
     *
     * <p>
     * {@link MapBinder#newMapBinder(com.google.inject.Binder, Class,
     ...
     * {@link com.google.inject.binder.LinkedBindingBuilder
     * LinkedBindingBuilder} 对象
     * </p>
     *
     * <p>
     * {@link MapBinder#addBinding(Object)} 用于添加一个绑定关系, 并给出一个
     * {@code Key} 值作为标识, 返回一个
    ...
    protected void configure() {
        // 一个绑定多个整数实例的 Set 集合
        var boundSet = Multibinder.newSetBinder(
            binder(),
            Integer.class,
        ...
        // 一个绑定多个实例的 Map 集合
        var boundMap = MapBinder.newMapBinder(
            binder(),
            String.class,
            BindDemo.class,
            Names.named("boundMap"));
        boundMap.addBinding("A")
                .toInstance(new BindDemoImpl("toInstanceA")); // 将对象实例绑定在 Key A 上
        boundMap.addBinding("B")
                .to(BindDemoImpl.class); // 将类型绑定在 Key B 上
        boundMap.addBinding("C")
                .toProvider(() -> new BindDemoImpl("toInstanceC")) // 将 Provider
                .asEagerSingleton();                                     // 绑定在 Key C 上
    }
