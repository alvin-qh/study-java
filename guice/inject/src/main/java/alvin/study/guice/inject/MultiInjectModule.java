 ...
 * 的边界也不同, 不会相互干扰. {@link com.google.inject.Module#configure(com.google.inject.Binder)
 ...
 * 是 {@link com.google.inject.Module#configure(com.google.inject.Binder) Module.configure(Binder)}
 * 方法的一个简化操作版本
 * </p>
 *
 * <p>
 ...
 * </p>
 *
 * @see alvin.study.bind.MultiBinderModule
 */
public class MultiInjectModule extends AbstractModule {
    /**
     * 配置模块
     *
     * <p>
    ...
    protected void configure() {
        ...
        boundSet.addBinding().toInstance(100);
        boundSet.addBinding().toInstance(200);
        boundSet.addBinding().toProvider(() -> 300);

        // 一个绑定多个实例的 Map 集合
        var boundMap = MapBinder.newMapBinder(binder(), String.class, InjectDemo.class, Names.named("boundMap"));
        boundMap.addBinding("A").toInstance(new InjectDemo("toInstanceA"));
        boundMap.addBinding("B").to(InjectDemo.class);
        boundMap.addBinding("C").toProvider(() -> new InjectDemo("toInstanceC"));
    }
}
