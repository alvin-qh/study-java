...
import alvin.study.app.endpoint.model.AppInfoDto;
import alvin.study.app.endpoint.model.AuthDto;
...
class PredicatesTest extends IntegrationTest {
    ...
    void path_shouldPredicatesWorked() {
        ...
        var resp = restTemplate.exchange(
            "http://study-springcloud-gateway/backend/api/info",
            HttpMethod.GET,
            null,
        ...
        then(resp.getPath()).isEqualTo("/api/info");

        // 确认相应内容符合预期
        var payload = resp.getPayload();
        then(payload.getApplicationName()).isEqualTo("study-springcloud-gateway-backend");
        then(payload.getLocalPort()).isIn(8081, 8082);
    }
    ...
    void jwt_shouldPredicatesWorked() {
        ...
        // 发起请求
        var resp = restTemplate.exchange(
            "http://study-springcloud-gateway/auth",
            HttpMethod.GET,
        ...
        // 确认响应正确
        then(resp.getRetCode()).isZero();
        then(resp.getPath()).isEqualTo("/auth");
