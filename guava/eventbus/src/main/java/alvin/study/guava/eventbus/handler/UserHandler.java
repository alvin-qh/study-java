...
public class UserHandler extends AbstractHandler {
     ...
     *
     * <p>
     * {@code async} 参数为 {@code true} 会导致事件处理过程中模拟
     * IO 阻塞约 2 秒钟, 同时, 必须使用
     * {@link EventBusManager#registerAsyncEventBus(String)}
     * 方法创建 {@code EventBus} 对象
     * </p>
     *
     ...
     *
     * <p>
     * 本方法会对 {@link UserEvent} 类型的事件进行处理, 即将 {@link UserEvent}
     * 对象作为方法参数
     * </p>
     *
     * <p>
     * 所有通过 {@link EventBus#post(Object)} 方法, 且参数为 {@link UserEvent}
     * 类型对象的调用, 都会执行到此方法中, 从而完成了该方法和其它方法的解耦, 参考
     * {@link alvin.study.guava.eventbus.repository.UserRepository#insertUser(User)
     * UserRepository.insertUser(User)} 方法中对事件进行发布
     * </p>
     *
     * <p>
     * 此方法有可能和发布方在一个线程中执行 (同步方式), 也有可能和发布方在不同线程执行 (异步方式),
     * 主要看创建 {@link EventBus} 对象的方式, 参考
     * {@link EventBusManager#registerEventBus(String)} 和
     * {@link EventBusManager#registerAsyncEventBus(String)} 方法
     * </p>
     *
     * <p>
     * 对于异步方式创建的 {@link EventBus} 对象, 且 {@link #asAsync} 字段为 {@code true} 时,
     * 本方法会模拟 IO 阻塞约 2 秒, 以体现异步消息处理的特征
     * </p>
     *
    ...
    public void onUserCreated(@Nonnull UserEvent event) {
        ...
        Preconditions.checkArgument(
            user.getId() > 0, "User.id property must great than 0");
