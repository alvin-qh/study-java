...
class OptionalTest {
     ...
     *
     * <p>
     * {@link Optional#of(Object)} 方法可以将一个非 {@code null} 的引用包装为 {@link Optional} 对象
     * </p>
     *
     * <p>
     * {@link Optional} 对象的 {@link Optional#isPresent()} 方法返回其包装的引用是否不为 {@code null} 值
     * </p>
     *
     * <p>
     ...
     *
     * <p>
     * {@link Optional#fromNullable(Object)} 方法可以将一个可能 {@code null} 的引用包装为 {@link Optional} 对象
     * </p>
     *
     * <p>
     * {@link Optional#absent()} 方法直接产生一个包含 {@code null} 引用的 {@link Optional} 对象
     * </p>
     *
     * <p>
     * 如果 {@link Optional} 对象存储的引用为 {@code null}, 则 {@link Optional#get()} 方法会引发异常
     * </p>
     *
     * <p>
     * {@link Optional#orNull()} 方法可以在 {@link Optional} 对象包含 {@code null} 引用时返回 {@code null} 值, 否则返回其存储
     * 的引用值, 在不强制要求从 {@link Optional} 中获取的引用是否为 {@code null} 时, 可采用此方法
     * </p>
     *
     * <p>
     * {@link Optional} 对象的 {@code or} 系列方法提供了在对象存储引用为 {@code null} 时, 如何返回另一个值, 包括:
     * <ul>
     * <li>
     ...
     * </li>
     * <li>
     * {@link Optional#or(com.google.common.base.Supplier) Optional.or(Supplier)} 方法, 返回非 {@code null} 引用值或所给
     * Lambda 表达式的返回值
     * </li>
     * </ul>
     * </p>
    ...
    /**
     * 将 {@link Optional} 对象中的引用放入一个 {@link Set} 集合中
     *
     * <p>
     * {@link Optional#asSet()} 方法返回一个只读的, 只包含一个元素的集合, 该方法的主要作用是可以通过迭代循环来保证循环体内的代码只有在
     * {@link Optional} 对象中存储的引用不为 {@code null} 时执行
     * </p>
     *
     * <p>
     * 对于 JDK 8 及以上的版本, 可以通过 {@link java.util.Optional#ifPresent(java.util.function.Consumer)
     * Optional.ifPresent(Consumer)} 方法来达到同样的目的
     * </p>
     *
     * <p>
     * 对于 JDK 9 及以上的版本, 可以通过 {@link java.util.Optional#stream() Optional.stream()} 方法得到一个
     * {@link java.util.stream.Stream Stream} 对象, 通过该对象达成同样的目的
     * </p>
     *
     * <p>
     * 可以通过 {@link Optional#toJavaUtil()} 方法将对象转为 JDK 的 {@link java.util.Optional Optional} 类型对象
     * </p>
     */
    @Test
     ...
     *
     * <p>
     * 所谓展开, 即将一个 {@code Collection<Optional<T>>} 类型的集合展开为 {@code Collection<T>} 类型
     * </p>
     *
     * <p>
     * 在 JDK 8 及以上版本, 可以通过 {@link java.util.stream.Stream#map(java.util.function.Function)
