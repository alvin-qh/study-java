 ...
 *
 * <p>
 * 通过 {@link RateLimiter#create(double, long, TimeUnit)} 方法可以创建 {@link RateLimiter}
 * 对象, 其参数为:
 * <ul>
 * <li>
...
class RateLimiterTest {
     ...
     *
     * <p>
     * 通过 {@link RateLimiter#tryAcquire(int, long, TimeUnit)} 类型对象限制指定方法每秒的调用次数,
     * 其中:
     * <ul>
     * <li>
    ...
    void limit_shouldLimitRateInPerSecond() {
        // 定义一个允许每秒调用 500 次的调用速率限制器对象
        var rateLimiter = RateLimiter.create(500.0);
        ...
        // 按每秒 1000 次调用指定函数, 持续 300ms
        callByRate(1000, 300, TimeUnit.MILLISECONDS, () -> {
        ...
        // 确认整个过程中, 允许调用 150 次左右, 即 3 * 50 = 150
        then(acquired.get()).isGreaterThanOrEqualTo(150L).isLessThan(155L);
     ...
     *
     * <p>
     * 调用频率限制的预热指的是一个调用点从"冷"到"热"的过程, 在大部分时候,
     * 第一次调用某个接口或者长时间不调用开始调用时, 其缓存 (包括数据库和内存缓存),
     * 以及其它资源可能都会在准备过程中, 此时系统可能无法承受突然爆发的 QPS,
     * 所以 {@link RateLimiter} 允许有一个预热时间, 在此时间过程中,
     * 逐步达到稳定的最大限制值, 例如: 预热时间为 3 秒, 最终的 QPS 限制在 50,
     * 则第一秒的 QPS 可能被限制在 20, 第二秒为 25 ... 直到在规定时间内达到最高的 50
     * </p>
     */
    @Test
    void limit_shouldLimitRateInPerSecondAndWarmup() {
        // 定义一个允许每秒调用 500 次的调用速率限制器对象 (即平均每毫秒 1 次), 预热时间 50ms
        var rateLimiter = RateLimiter.create(500.0, 50, TimeUnit.MILLISECONDS);
        ...
        // 设置 4 阶段调用, 每个阶段持续 100ms 时间
        for (var i = 0; i < 4; i++) {
            // 记录总共调用次数和成功调用数量
            var total = new AtomicLong();
            var acquired = new AtomicLong();

            // 按每秒 1000 次调用指定函数, 持续 100ms
            callByRate(1000, 100, TimeUnit.MILLISECONDS, () -> {
                total.incrementAndGet();
        ...
        then(records).extracting("total")
                .allMatch(total -> (long) total >= 80 && (long) total <= 100);
