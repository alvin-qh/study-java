 ...
 *
 * <p>
 * 所谓代码执行前置条件包括: 方法参数是否合法, 变量是否为 {@code null},
 * 集合索引值是否正确, 包括:
 * <ul>
 * <li>
 ...
 * </li>
 * <li>
 * {@link Preconditions#checkElementIndex(int, int, String)},
 * 检查下标索引是否在集合长度允许范围内
 * </li>
 * <li>
 * {@link Preconditions#checkPositionIndex(int, int, String)},
 * 检查位置索引是否在集合长度允许范围内
 * </li>
 * </ul>
 * </p>
 *
 * <p>
 * {@link Preconditions} 类的某些方法已经被 JDK 类似方法取代,
 * 例如 {@link java.util.Objects#requireNonNull(Object)
 * Objects.requireNonNull(Object)},
 * {@link java.util.Objects#checkIndex(int, int)
 * Objects.checkIndex(int, int)} 以及
 * {@link java.util.Objects#checkFromIndexSize(int, int, int)
 * Objects.checkFromIndexSize(int, int, int)} 等
 * </p>
 */
class PreconditionsTest {
     ...
     * <p>
     * {@link Preconditions#checkArgument(boolean, String, Object...)}
     * 方法的第一个参数为条件表达式, 后续参数为一个错误信息模板以及模板参数,
     * 如果所给的条件表达式为 {@code false}, 则抛出
     * {@link IllegalArgumentException} 异常, 并携带所给的错误信息
     * </p>
     *
     * <p>
    ...
    void argument_shouldCheckArgumentByCondition() {
        ...
        thenThrownBy(() -> Preconditions.checkArgument(arg > 10))
                .isInstanceOf(IllegalArgumentException.class);

        thenThrownBy(
            () -> Preconditions.checkArgument(arg > 10, "Expect n > 10, but n = %s", arg))
                    .isInstanceOf(IllegalArgumentException.class)
     ...
     * {@link Preconditions#checkState(boolean, String, Object...)}
     * 方法的第一个参数为条件表达式, 后续参数为一个错误信息模板以及模板参数.
     * 如果所给的条件表达式为 {@code false}, 则抛出 {@link IllegalStateException}
     * 异常, 并携带所给的错误信息
     * </p>
     *
     * <p>
    ...
    void state_shouldCheckStateByCondition() {
        ...
        thenThrownBy(() -> Preconditions.checkState(state == 0))
                .isInstanceOf(IllegalStateException.class);

        thenThrownBy(
            () -> Preconditions.checkState(state == 0, "Expect n is zero"))
                    .isInstanceOf(IllegalStateException.class)
     ...
     * {@link Preconditions#checkNotNull(Object, String, Object...))}
     * 方法的第一个参数为条件表达式, 后续参数为一个错误信息模板以及模板参数.
     * 如果所给参数引用为 {@code null}, 则抛出 {@link NullPointerException} 异常,
     * 并携带所给的错误信息
     * </p>
     *
     * <p>
     * 如果所给参数不为 {@code null}, 则 {@link Preconditions#checkNotNull(Object)}
     * 方法的返回值是所给参数引用本身
     * </p>
     *
     * <p>
    ...
    void nullCheck_shouldCheckReferenceIfNullOrNot() {
        ...
        // 检查一个引用是否不为 null
        thenThrownBy(() -> Preconditions.checkNotNull(nullValue))
                .isInstanceOf(NullPointerException.class);

        thenThrownBy(
            () -> Preconditions.checkNotNull(nullValue, "Expect obj not null"))
                    .isInstanceOf(NullPointerException.class)
     ...
     * <p>
     * {@link Preconditions#checkElementIndex(int, int, String)}
     * 方法的第一个参数为索引值, 第二个参数为集合元素个数, 第三个参数为所检查索引值的简单描述.
     * 如果第一个参数的值不在所给集合元素个数范围内, 则抛出 {@link IndexOutOfBoundsException} 异常
     * </p>
     *
     * <p>
     * 如果所给的索引值正确, 则 {@link Preconditions#checkElementIndex(int, int, String)}
     * 方法返回索引值本身
     * </p>
     */
    @CsvSource({
     ...
     *
     * <p>
     * 如果所给的索引值正确, 则 {@link Preconditions#checkPositionIndex(int, int, String)}
     * 方法返回索引值本身
     * </p>
     */
    @CsvSource({
