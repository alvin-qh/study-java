...
class MonitorTest {
     ...
     *
     * <p>
     * 代码中设置了 {@code 100} 个线, 每个线程将一个公共的 {@link Counter}
     * 对象的计数值增加 {@code 1000} 次
     * </p>
     *
     * <p>
     * 所以本例中一共对计数器对象进行了 {@code 100 * 1000} 次的递增操作,
     * 但实际的结果却小于这个数字, 这是由于多线程在操作同一内存区域时,
     * 造成的访问问题
     * </p>
     */
    @RetryingTest(3)
     ...
     *
     * <p>
     * 通过 {@link Monitor#enter()} 方法可以进入一个无条件锁, 进入锁的线程将独占该锁,
     * 直到调用了 {@link Monitor#leave()} 方法提出对锁的占用
     * </p>
     *
     * <p>
     * 通过 {@link Monitor#enter()} 方法可以进入一个无条件锁, 进入锁的线程将独占该锁,
     * 直到调用了 {@link Monitor#leave()} 方法结束对锁的占用
     * </p>
     *
     * <p>
     * 本例的操作和 JDK 中 {@link java.util.concurrent.locks.Condition Condition}
     * 类型的使用非常类似
     * </p>
     */
    @Test
