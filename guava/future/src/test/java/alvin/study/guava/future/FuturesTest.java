...
class FuturesTest {
     ...
     * Futures.addCallback(ListenableFuture, FutureCallback, Executor)} 方法可以向线程池提交一个任务, 并在任务结束后进行回调, 其中:
     * <ul>
     * <li>
     * 参数 1 是通过 {@link com.google.common.util.concurrent.ListeningExecutorService#submit(java.util.concurrent.Callable)
     ...
     * ListeningExecutorService.submit(Runnable)} 方法 (无返回值) 获得, 为一个 {@link ListenableFuture} 类型对象
     * </li>
     * <li>
     * 参数 2 是一个 {@link FutureCallback} 接口对象, 用于对执行结果进行处理, 其中 {@link FutureCallback#onSuccess(Object)}
     * 表示对执行成功的结果进行处理, {@link FutureCallback#onFailure(Throwable)} 表示对执行失败的异常进行处理
     * </li>
     * <li>
     * 参数 3 是一个 {@link java.util.concurrent.Executor Executor} 类型对象, 表示用于执行 {@link FutureCallback}
     * 接口的线程执行器
     * </li>
     * </ul>
     * </p>
     ...
     *
     * <p>
     * 通过 {@link Futures#allAsList(Iterable)} 方法可以将一系列 {@link ListenableFuture} 集合转为一个 {@link ListenableFuture}
     * 对象, 该 {@link ListenableFuture} 对象的执行结果为前面所有 {@link ListenableFuture} 对象执行结果的集合
     * </p>
     */
    @Test
     ...
     *
     * <p>
     * 通过 {@link ListenableFuture#addListener(Runnable, java.util.concurrent.Executor)
     * ListenableFuture.addListener(Runnable, Executor)} 方法可以为 {@code Future} 对象增加一个监听器,
     * 当异步任务执行完毕后会回调监听器
     * </p>
     */
    @Test
     ...
     *
     * <p>
     * 通过 {@link Futures#transform(ListenableFuture, com.google.common.base.Function, java.util.concurrent.Executor)
     * Futures.transform(ListenableFuture, Function, Executor)} 方可对一个 {@link ListenableFuture} 的结果类型进行转化,
     * 其中参数 2 指定了转化函数, 参数 3 指定了执行转化函数的线程运行器
     * </p>
     *
     * <p>
     * {@link Futures#transformAsync(ListenableFuture, com.google.common.util.concurrent.AsyncFunction, java.util.concurrent.Executor)
     * Futures.transformAsync(ListenableFuture, AsyncFunction, Executor)} 方法的作用和 {@code transform} 的作用基本类似,
     * 只是后者对 {@link ListenableFuture} 对象的执行和转化是同步进行的, 必须一次性完成; 而前者则是将转换也变成是一个异步任务的
     * {@link ListenableFuture} 对象
     * </p>
     *
     * <p>
     * 和传统的转换方法不同, {@code transform} 和 {@code transformAsync} 可以看作是链式调用, 即这两个方法提供了一个异步任务,
     * 其参数是前一个异步任务的结果
     * </p>
     */
    @Test
