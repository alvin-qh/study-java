...
/**
 * {@link Collections2} 类型是 Guava 对 {@link java.util.Collections}
 * 工具类的补充
 *
 * <p>
 ...
 *
 * <p>
 * 如果使用的是 JDK 8 以上的版本, Guava 推荐使用 Java 本身提供的 Stream 库,
 * {@link Collections2} 类的方法不会过期停用, 但已经不推荐在 JDK 8
 * 及以上版本使用
 * </p>
 */
class CollectionUtilsTest {
     ...
     * Collections2.filter(Collection, Predicate)} 方法返回一个
     * {@code Collections2.FilteredCollection} 类型对象, 该对象代理了原集合对象,
     * 在获取元素时根据过滤条件返回符合条件的结果, 所以如果原集合发生变化,
     * 可能会导致过滤结果也发生变化
     * </p>
     */
    @Test
     ...
     *
     * <p>
     * {@link Collections2#permutations(java.util.Collection)
     * Collections2.permutations(Collection)} 方法返回一个
     * {@code Collections2.PermutationCollection} 类型对象,
     * 该对象代理了原集合对象, 在获取元素时根据过滤条件返回符合条件的结果,
     * 所以如果原集合发生变化, 可能会导致过滤结果也发生变化
     * </p>
     *
     * <p>
     * 计算全排列的算法如下:
     ...
     * <li>
     * {@link Collections2#permutations(java.util.Collection)
     * Collections2.permutations(Collection)} 方法返回
     * {@code Collections2.PermutationCollection} 类型对象,
     * 该对象通过 {@code Collections2.PermutationIterator}
     * 类型迭代器来产生集合元素的各种排列组合, 迭代器每次迭代返回一个
     * {@code List<T>} 集合, 表示一种排列组合
     * </li>
     * <li>
     * {@code Collections2.PermutationIterator} 迭代器每次迭代,
     * 交换集合中两个位置位置的元素, 以产生一种排列组合,
     ...
     * {@link Collections2#permutations(java.util.Collection)
     * Collections2.permutations(Collection)} 方法基本类似,
     * 区别在于:
     * <ul>
     * <li>
    ...
    void orderedPermutations_shouldFindElementsMatchedConditionInIterables() {
        ...
        // 对集合进行过滤, 获取过滤结果
        var permutationList = Collections2.orderedPermutations(
            coll, Ordering.natural().reversed());
     ...
     * 方法通过一个转换函数对集合中的元素进行转换, 返回包含转换后元素的集合对象
     * </p>
     *
     * <p>
     * 该方法返回结果为 {@code Collections2.TransformedCollection} 类型,
     * 是一个原集合对象的代理对象, 在获取元素时才会对原集合元素进行转换处理,
     * 所以对于原集合的修改, 可能会引起结果集合的变化
     * </p>
     */
    @Test
    void transform_shouldTransformElementsToOther() {
        ...
        var transformedList = Collections2.transform(
            coll, n -> String.format("0%d", n));
        // 确认全部元素已被转换
