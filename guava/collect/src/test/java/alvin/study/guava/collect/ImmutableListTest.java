 ...
 * 它本质上是对现有集合对象的一个代理, 只是禁用了修改集合元素的一系列方法, 作用是保障代码安全;
 * 而 Guava 提供的不变集合则是从数据结构上不支持修改, 除保障代码安全外, 还可以提供更好的执行
 * 效率和内存空间使用率, 且无需担心线程安全问题
 * </p>
 *
 * <p>
 * 本例中以 {@link ImmutableList} 类型为例, 演示不变集合类型的使用,
 * {@link com.google.common.collect.ImmutableSet ImmutableSet}
 * 等集合类型和其使用方法基本类似
 * </p>
 *
 * <p>
 * 注意, 基于设计原因, {@link ImmutableList} 等集合类型不允许使用 {@code null}
 * 作为元素值
 * </p>
 *
 * <p>
 * JDK 9 以后, JDK 本身提供了 {@link java.util.List#of(Object) List.of(T...)}
 * 方法, 可以取代 Guava 库的使用
 * </p>
 */
class ImmutableListTest {
     ...
     *
     * <p>
     * {@link ImmutableList#of(Object) ImmutableList.of(Object...)}
     * 方法可以指定多个元素, 构建一个不变 {@code List} 集合
     * </p>
     *
     * <p>
     * {@code ImmutableList.of(...)} 方法具备多个重载形式,
     * 可以支持 1 ~ n 个参数的情况
     * </p>
     */
    ...
    void of_shouldCreateImmutableListByElements() {
        ...
        // 确认集合不支持修改操作
        thenThrownBy(() -> list.add(5))
                .isInstanceOf(UnsupportedOperationException.class);

        // 确认元素不能包含 null 值
        thenThrownBy(() -> ImmutableList.of(1, null))
     ...
     *
     * <p>
     * {@link ImmutableList#copyOf(Iterable)} 可以创建现有集合的一个不变副本,
     * 该方法也具备多个重载形式, 可以支持集合的多种形式参数
     * </p>
     */
    @Test
    void copyOf_shouldCreateImmutableListFromOtherCollection() {
        ...
        src.add(null);
        thenThrownBy(() -> ImmutableList.copyOf(src))
                .isInstanceOf(NullPointerException.class);
    }
    ...
    void copyOf_shouldCreateSortedImmutableListFromOtherCollection() {
        ...
        then(result).isNotSameAs(src);
        then(result).containsExactlyInAnyOrderElementsOf(src)
                .isSortedAccordingTo((l, r) -> r - l);
    }
     ...
     *
     * <p>
     * {@link ImmutableList#builder()} 可以创建一个 {@link ImmutableList.Builder}
     * 对象, 通过该对象可以分批设置集合元素, 最终构建不变集合对象
     * </p>
     */
    @Test
     ...
     *
     * <p>
     * 预期元素个数无需精确, 如果最终添加的元素个数超出预期个数, 也不会导致错误,
     * 只是会导致存储区重建
     * </p>
     */
    @Test
