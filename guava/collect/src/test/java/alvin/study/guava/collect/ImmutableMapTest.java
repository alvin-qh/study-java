 ...
 *
 * <p>
 * 和 {@link java.util.Collections#unmodifiableMap(java.util.Map)
 * Collections.unmodifiableMap(Map)} 等方法包装的不可修改 Map 不同,
 * 它本质上是对现有集合对象的一个代理, 只是禁用了修改集合元素的一系列方法,
 * 作用是保障代码安全; 而 Guava 提供的不变 Map 则是从数据结构上不支持修改,
 * 除保障代码安全外, 还可以提供更好的执行效率和内存空间使用率,
 * 且无需担心线程安全问题
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * 注意, 基于设计原因, {@link ImmutableMap} 不允许使用 {@code null}
 * 作为 Key 和 Value 值
 * </p>
 *
 * <p>
 * JDK 9 以后, JDK 本身提供了 {@link java.util.Map#of(Object, Object)
 * Map.of(K1, V1, ..., Vn, Vn)} 方法, 可以取代 Guava 库的使用
 * </p>
 */
class ImmutableMapTest {
     ...
     *
     * <p>
     * {@link ImmutableMap#of(Object, Object)} 方法具备若干重载形式,
     * 通过一系列参数, 按照 {@code K1, V1, K2, V2, ..., Kn, Vn}
    ...
    void of_shouldCreateImmutableMap() {
        ...
        thenThrownBy(() -> map.put("C", 300))
                .isInstanceOf(UnsupportedOperationException.class);

        // 确认 ImmutableMap 对象不能包含值为 null 的 Key
        thenThrownBy(() -> ImmutableMap.of(null, 100))
                .isInstanceOf(NullPointerException.class);
        // 确认 ImmutableMap 对象不能包含值为 null 的 Value
        thenThrownBy(() -> ImmutableMap.of("C", null))
                .isInstanceOf(NullPointerException.class);
    }
     ...
     *
     * <p>
     * {@link ImmutableMap#ofEntries(Map.Entry...)} 通过一系列
     * {@link Map.Entry} 类型参数构建 Map 对象
     * </p>
     */
    @Test
    void ofEntries_shouldCreateImmutableMap() {
        ...
        thenThrownBy(() -> ImmutableMap.ofEntries(Map.entry(null, 300)))
                .isInstanceOf(NullPointerException.class);
        // 确认 ImmutableMap 中不能包含 Value 为 null 的 Entry
        thenThrownBy(() -> ImmutableMap.ofEntries(Map.entry("C", null)))
                .isInstanceOf(NullPointerException.class);
    }
     ...
     *
     * <p>
     * {@link ImmutableMap#copyOf(Map)} 方法创建一个 {@link java.util.Map Map}
     * 对象的不变副本
     * </p>
     */
    @Test
    void copyOf_shouldCreateImmutableMap() {
        ...
        src.put(null, 300);
        thenThrownBy(() -> ImmutableMap.copyOf(src))
                .isInstanceOf(NullPointerException.class);
     ...
     *
     * <p>
     * {@link ImmutableMap#builder()} 可以创建一个 {@link ImmutableMap.Builder} 对象,
     * 通过该对象可以分批设置键值对, 最终构建不变 Map 对象
     * </p>
     *
     * <p>
     * 另外, {@link ImmutableMap#builderWithExpectedSize(int)} 可以设置一个预期的键值对数量,
     * 提前分批足够的内存, 减少构建过程中重建存储内存的性能影响.
     * 预期的键值对数量不会导致实际添加键值对时发生失败, 不当的设置只会导致内存浪费或出现存储区重建情况
     * </p>
     *
     * <p>
     * 注意: {@link ImmutableMap.Builder#build()} 方法内部调用了
     * {@link ImmutableMap.Builder#buildOrThrow()} 方法, 在添加键值对时, 如果出现重复的键值,
     * 则会导致抛出异常
     * </p>
     */
     ...
     *
     * <p>
     * {@link ImmutableMap.Builder#buildKeepingLast()} 执行的策略是:
     * 如果在构建过程中出现重复的键, 则保留最后一次添加的键值对, 覆盖之前添加的键值对
     * </p>
     *
     * <p>
