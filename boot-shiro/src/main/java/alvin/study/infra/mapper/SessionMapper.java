...
import alvin.study.infra.entity.Session;
...
public interface SessionMapper extends BaseMapper<Session> {
    /**
     * 根据 Session key 查询 Session 值
     *
     * @param key Session key
     * @return Session 的值
     */
    @Select("""
        select id, `key`, `value`, expired_at, created_by, created_at, updated_by, updated_at
        from session
        where `key` = #{key}
        """)
    Optional<Session> selectByKey(@Param("key") String key);
}
