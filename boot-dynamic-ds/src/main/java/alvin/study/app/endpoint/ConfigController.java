...
import alvin.study.app.domain.service.ConfigService;
import alvin.study.app.endpoint.model.ConfigDto;
...
public class ConfigController {
    ...
    ResponseDto<ConfigDto> createConfig(@NotBlank @PathVariable("org") String org) {
        // 创建配置实体对象
        var entity = configService.createConfig(org);

        // 将 Entity 转为 Dto 后包装为成功结果
        return ResponseDto.success(new ConfigDto(
            entity.getId(),
            entity.getOrg(),
            entity.getDbName(),
            entity.getUpdatedAt(),
