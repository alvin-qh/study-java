...
import alvin.study.app.domain.service.DataService;
import alvin.study.app.endpoint.model.DataDto;
import alvin.study.app.endpoint.model.DataForm;
import alvin.study.core.http.common.ResponseDto;
...
public class DataController {
    ...
    ResponseDto<DataDto> createData(@Valid @RequestBody DataForm form) {
        ...
        // 创建实体对象
        dataService.createData(entity);

        // 返回结果
        return ResponseDto.success(new DataDto(
            entity.getId(),
            entity.getName(),
            entity.getValue(),
            entity.getUpdatedAt(),
    ...
    ResponseDto<DataDto> getData(@NotNull @PathVariable("id") Long id) {
        try {
            // 获取实体对象
            var entity = dataService.getData(id);
