package alvin.study.app.web.controller;
...
import alvin.study.app.web.model.Form;
 ...
 * <p>
 * 上述涉及到的拦截器均是通过:
 * {@link alvin.study.conf.WebConfig#addInterceptors(org.springframework.web.servlet.config.annotation.InterceptorRegistry)
 * WebConfig.addInterceptors(InterceptorRegistry)} 方法进行注册
 * </p>
 *
 * <p>
 * 若某次请求携带了本地化设置参数或信息, 则会通过 {@link alvin.study.conf.WebConfig#localeResolver()
 * WebConfig.localeResolver()} 方法, 设置本地化信息的持久化方法, 本例中是持久化到 Cookie 中
 * </p>
 *
 * <p>
 * Controller 方法返回后, 下一步会通过模板引擎对对应的 HTML 进行渲染, 模板引擎的设置参考:
 * {@link alvin.study.conf.WebConfig#templateResolver()
 * WebConfig.templateResolver()}
 * </p>
 *
 * <p>
 * 对于浏览器的镜头资源请求 (js, css, image 等), 参见
 * {@link alvin.study.conf.WebConfig#addResourceHandlers(org.springframework.web.servlet.config.annotation.ResourceHandlerRegistry)
