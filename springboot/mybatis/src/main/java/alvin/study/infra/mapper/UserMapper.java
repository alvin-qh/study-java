...
public interface UserMapper extends BaseMapper<User> {
    /**
     * 根据 {@code account} 属性值查询 {@link User} 实体对象
     *
     * @param account 账号名
     ...
     * 通过
     * {@link BaseMapper#selectList(com.baomidou.mybatisplus.core.conditions.Wrapper)
     * BaseMapper.selectList(Wrapper)} 方法进行动态查询
     *
     * <p>
     * {@link QueryWrapper} 对象表示了一个查询条件的包装对象, 可以通过其中的一系列方法组合复杂条件对实体进行查询
     * </p>
     *
     * @param account 用户账号
     * @return 符合条件的用户列表
     */
    default List<User> selectAllByAccount(String account) {
        return selectList(Wrappers.<User>query().likeRight("account", account));
    }
}
