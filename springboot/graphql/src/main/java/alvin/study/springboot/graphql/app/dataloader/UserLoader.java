 ...
 *
 * <p>
 * 如果批量查询 {@link User} 类型实体对象时, 可以通过 {@link org.dataloader.DataLoader DataLoader} 类型对象进行优化,
 * {@link org.dataloader.DataLoader DataLoader} 类型查询的原理为:
 *
 * <p>
 * {@link org.dataloader.DataLoader DataLoader} 类型对象通过异步方式执行查询, 当批量查询 {@link User} 类型实体对象时,
 * 每个查询都会返回一个 {@link java.util.concurrent.CompletableFuture CompletableFuture} 类型对象, 当批量查询结束后,
 * 会将所有查询进行合并, 包括: 将查询条件合并为一个集合; 将查询给到 {@link UserLoader} 类型对象, 批量查询结果后,
...
public class UserLoader implements BiFunction<Set<Long>, BatchLoaderEnvironment, Mono<Map<Long, User>>> {
