package alvin.study.core.graphql.handler;
...
import alvin.study.core.exception.ErrorCode;
import alvin.study.core.exception.ErrorExtensionCode;
import alvin.study.core.exception.FieldError;
import alvin.study.core.exception.GraphqlBaseException;
...
public class GraphQLErrorHandler implements GraphQLQueryResolver {
    ...
    public GraphQLError handle(Exception e, ErrorContext errorContext) {
    ...
    public GraphQLError handle(GraphqlBaseException e, ErrorContext errorContext) {
        var err = makeGraphQLError(
    ...
    public GraphQLError handle(AccessDeniedException e, ErrorContext errorContext) {
        var message = e.getMessage() == null ? UNKNOWN_ERROR_REASON : e.getMessage();
        var err = makeGraphQLError(
    ...
    public GraphQLError handle(ConstraintViolationException e, ErrorContext errorContext) {
    ...
    public GraphQLError handle(NoSuchElementException e, ErrorContext errorContext) {
        var message = e.getMessage() == null ? UNKNOWN_ERROR_REASON : e.getMessage();
        var err = makeGraphQLError(
            errorContext,
            ErrorCode.NOT_FOUND,
    ...
    private static GraphQLError makeGraphQLError(
        ...
        if (errorContext.getExtensions() != null) {
            ...
            extensions.putAll(errorContext.getExtensions());
        }
